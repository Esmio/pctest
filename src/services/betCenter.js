...
export const getRandomPicks = ((gameSetCombination) => {
  const { 
    pickRange, sections, set, isUnique, minimumRowPick, isGroupBtn
  } = gameSetCombination;
...
export function getBetString(
  { thisBetObj }, { sections, set, joinPickWith, joinSectionWith, isGroupBtn }
) {
  let thisBetString = '';
  _.forEach(sections, (section, secIndex) => {
    const sectionJoiner = secIndex === sections.length - 1 ? '' : joinSectionWith || '|';
    if (thisBetObj[section]) {
      const bets = set[secIndex] || set[0];
      let sectionStr = '';
      _.forEach(bets, (bet, betIndex) => {
        const betStr = isGroupBtn ? bet.displayText : bet;
        const pickJoiner = betIndex === bets.length - 1 ? '' : joinPickWith || ' ';
